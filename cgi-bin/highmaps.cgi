...
foreach my $variete (sort keys(%hash_varietes))
{
	...
	if ($Configuration::CONF_INTERFACE == 0){
		$country_name = $hash_varietes{$variete}{"Country"};
        $hash_variety_country{$variete} = $hash_varietes{$variete}{"Country"};
	}
	if ($Configuration::CONF_INTERFACE == 1){
		$country_name = $hash_varietes{$variete}{"CountryofVarieties"};
        $hash_variety_country{$variete} = $hash_varietes{$variete}{"CountryofVarieties"};
	}
}
